...
#define ONENET_RECV_BUFFER_LENGTH   1200
...
s32 RIL_QONENET_Notify(ST_ONENET_Notify_Param_t* onenet_notify_param_t,bool ack_flag)
{
	...
	strAT = (u8*)Ql_MEM_Alloc(sizeof(u8)*1500);
	if(NULL == strAT)
	{
...
s32 RIL_QONENET_Write_Rsp(Onenet_Urc_Param_t* onenet_urc_param_ptr,Enum_ONENET_Observe_Result onenet_observe_result)
{

     s32 ret = RIL_AT_SUCCESS;
	 char strAT[200] ;

	 if(onenet_urc_param_ptr->index == 0)//Sending the write respond when  the last message of  write request is received.
	 {
    	 Ql_memset(strAT,0, sizeof(strAT));
    	 Ql_sprintf(strAT,"AT+MIPLWRITERSP=%d,%d,%d\n",onenet_urc_param_ptr->ref,onenet_urc_param_ptr->msgid,onenet_observe_result);
    
    	 ret = Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT), ATResponse_Handler,NULL,0);
    	 RIL_ONENET_DEBUG(DBG_Buffer,"<-- Send AT:%s,ret=(%d) -->\r\n",strAT,ret);
	 }
	 Ql_MEM_Free(onenet_urc_param_ptr);
	 onenet_urc_param_ptr = NULL;
	 if(RIL_AT_SUCCESS != ret)
	 {
		 return ret;
	 }
